...
export default function UpstageWeb3EvaluationPage() {
  ...
  const runSingleEval = async (testCase: TestCase) => {
    ...
    setTimeout(() => {
      ...
      const overallScore = (accuracy + relevance + correctness + proofValidity + (1-executionCost) + verificationSpeed) / 6;
  ...
  const runCustomEval = async () => {
    ...
    setTimeout(() => {
      ...
      if (customInput.toLowerCase().includes("verification") || customInput.toLowerCase().includes("proof")) {
        const txHash = "0x" + Array(64).fill(0).map(() => "0123456789abcdef"[Math.floor(Math.random() * 16)]).join('');
        setCustomOutput(customExpected || `Generated response with Web3 verification:\n\nResponse: ${customInput.length > 30 ? customInput.substring(0, 30) + "..." : customInput}\n\nVerification Proof: [Valid ZK-Proof Generated]\nTransaction Hash: ${txHash}\nVerification Status: CONFIRMED\nBlock Number: ${19000000 + Math.floor(Math.random() * 1000000)}\nChain ID: 1 (Ethereum Mainnet)`);
      } else {
        setCustomOutput(customExpected || `Standard inference result for prompt: "${customInput}"\n\nGenerated content would appear here. When you provide an expected output, it will be compared to this result.`);
      }
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card>
            <CardHeader className="bg-secondary/30 border-b">
              ...
              <CardDescription>
                Test cases for on-chain verification
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <div className="max-h-[400px] overflow-y-auto">
                {datasets.map(dataset => (
                  <div 
                    key={dataset.id}
                    className={`p-4 border-b cursor-pointer hover:bg-secondary/50 ${selectedDataset?.id === dataset.id ? 'bg-secondary/70' : ''}`}
                    onClick={() => handleSelectDataset(dataset)}
                  >
                    <div className="font-medium flex items-center">
                      {dataset.id === "web3-verification" && <Shield className="h-4 w-4 mr-2 text-primary" />}
                      {dataset.id === "fine-tuning" && <Cpu className="h-4 w-4 mr-2 text-primary" />}
                      {dataset.id === "multi-chain" && <Database className="h-4 w-4 mr-2 text-primary" />}
                      {dataset.id === "zkml-verification" && <Code className="h-4 w-4 mr-2 text-primary" />}
                      {!["web3-verification", "fine-tuning", "multi-chain", "zkml-verification"].includes(dataset.id) && 
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="text-sm space-y-2">
                    ...
                    {selectedDataset.testCases.some(tc => results[tc.id] && results[tc.id].verificationMetrics) && (
                      <>
                        <div className="border-t mt-3 pt-3">
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between text-xs">
                              ...
                              <span>
                                {(selectedDataset.testCases
                                  .filter(tc => results[tc.id] && results[tc.id].verificationMetrics)
                                  .reduce((sum, tc) => sum + (results[tc.id].verificationMetrics?.proofValidity || 0), 0) / 
                                  selectedDataset.testCases.filter(tc => results[tc.id] && results[tc.id].verificationMetrics).length * 100).toFixed(1)}%
                              </span>
                            </div>
                            <div className="flex justify-between text-xs">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="datasets" className="space-y-4">
              {selectedDataset ? (
                      ...
                      <Card key={testCase.id} className="mb-4">
                        ...
                        <CardFooter>
                          <Button 
                            ...
                            variant="outline"
                          >
                            {isRunning ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Shield className="h-4 w-4 mr-2" />}
                            Run Verification
                          </Button>
                        </CardFooter>
                      </Card>
                ...
                <div className="p-8 text-center border rounded-md">
                  <p className="text-muted-foreground">Select a verification dataset from the sidebar to view test cases.</p>
                </div>
              )}
            </TabsContent>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Example: "Generate a response to 'Explain zkML' with zk-proof verification on Ethereum mainnet"
                    </p>
                  </div>
                  
                  <div>
                    <Label htmlFor="custom-expected">Expected Output with Verification Data (Optional)</Label>
                    <Textarea 
                      id="custom-expected"
                ...
                <CardContent className="space-y-4">
                  {selectedDataset ? (
                    <>
                      ...
                      <div>
                        <Label htmlFor="test-expected">Expected Output with Verification Data</Label>
                        <Textarea 
                          id="test-expected"
